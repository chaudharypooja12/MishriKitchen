  ...
  return (
    <section className="relative min-h-[100svh] overflow-hidden">
      <Image
        ...
        sizes="100vw"
      />
      <div className="absolute inset-0 bg-gradient-to-b from-slate-950/80 via-violet-950/50 to-slate-950/90" />
      ...
      <div className="relative z-10 flex min-h-[100svh] flex-col justify-center px-4 pb-40 pt-28 md:pb-36">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <h1 className="font-display text-4xl font-bold leading-tight tracking-tight text-white drop-shadow-lg md:text-6xl md:leading-[1.06]">
            Crafting Premium Kitchens & Interiors That Feel Like Home
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-slate-200/95 md:text-xl">
          ...
          <div className="glass-panel mx-auto grid max-w-5xl grid-cols-2 gap-3 p-4 md:grid-cols-4 md:gap-4 md:p-6">
            {heroStats.map((s) => (
              <div key={s.label} className="text-center">
                <p className="font-display bg-gradient-to-br from-white to-cyan-200 bg-clip-text text-2xl font-bold text-transparent md:text-3xl">
                  {s.value}
                </p>
                <p className="text-xs text-slate-300 md:text-sm">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
